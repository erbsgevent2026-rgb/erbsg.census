...
export const AnnualReportsView: React.FC = () => {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete Annual Report: ${name}?`)) return;
    try {
      await api.deleteAnnualReport(id);
      setStatusMessage({ type: "success", text: "Annual report deleted successfully." });
      await fetchReports();
    } catch (err: any) {
  ...
  const handleViewPdf = async (report: StatutoryDocument) => {
    try {
      const fullDoc = await api.getAnnualReportFile(report.id);
                      ...
                      <tr key={report.id} className="hover:bg-slate-50 dark:hover:bg-slate-750">
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <button
                            ...
                            className="p-1.5 bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300 hover:bg-blue-100 rounded-md transition"
                            title="View PDF"
                          >
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <button
                            onClick={() => printStatutoryDocument(report, "Annual Report", yearObj?.label || report.year_id)}
